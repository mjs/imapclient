...
class TestParseResponse(unittest.TestCase):
    ...
    def test_literal(self):
        literal_text = add_crlf(dedent("""\
            ...
            abc def XYZ
            """))
        self._test([(b'{18}', bytes(literal_text, 'latin-1'))], literal_text)
    ...
    def test_literal_with_more(self):
        literal_text = add_crlf(dedent("""\
            ...
            abc def XYZ
            """))
        response = [(b'(12 "foo" {18}', bytes(literal_text, 'latin-1')), b")"]
        self._test(response, (12, 'foo', literal_text))


    def test_quoted_specials(self):
        self._test(br'"\"foo bar\""', '"foo bar"')
        self._test(br'"foo \"bar\""', 'foo "bar"')
        self._test(br'"foo\\bar"', r'foo\bar')
...
class TestParseFetchResponse(unittest.TestCase):
    ...
    def test_BODY_HEADER_FIELDS(self):
        ...
        self.assertEqual(parse_fetch_response(
            [(b'123 (UID 31710 BODY[HEADER.FIELDS (from subject)] {57}', bytes(header_text, 'latin-1')), b')']),
    ...
    def check_BODYish_single_part(self, respType):
        text =  bytes('123 (UID 317 %s ("TEXT" "PLAIN" ("CHARSET" "us-ascii") NIL NIL "7BIT" 16 1))' % respType, 'latin-1')
    ...
    def check_BODYish_multipart(self, respType):
        text = bytes('123 (UID 269 %s (("TEXT" "HTML" ("CHARSET" "us-ascii") NIL NIL "QUOTED-PRINTABLE" 55 3)' \
    ...
    def test_partial_fetch(self):
        ...
        self.assertEqual(parse_fetch_response(
            [(b'123 (UID 367 BODY[]<0> {20}', bytes(body, 'latin-1')), b')']),
            { 367: {'BODY[]<0>': body,
                    'SEQ': 123}})
    ...
    def test_INTERNALDATE_normalised(self):
        def check(date_str, expected_dt):
            output = parse_fetch_response([bytes('3 (INTERNALDATE "%s")' % date_str, 'latin-1')])
    ...
    def test_INTERNALDATE(self):
        def check(date_str, expected_dt):
            output = parse_fetch_response([bytes('3 (INTERNALDATE "%s")' % date_str, 'latin-1')], normalise_times=False)
            actual_dt = output[3]['INTERNALDATE']
            self.assertEqual(actual_dt, expected_dt)
